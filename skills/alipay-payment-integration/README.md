...
- **Full-Scenario Payment Products** — Covers Face-to-Face Payment, Order Code Payment, APP Payment, JSAPI Payment, Mobile Website Payment, PC Website Payment, Pre-Auth Payment, and Merchant Debit
...
| Merchant Debit | `alipay.trade.app.pay` + `alipay.trade.pay` | Membership subscription, auto-renewal, periodic deduction |
...
| 1.2 Sandbox Setup | Create sandbox environment | CLI tool for Unix/macOS/Linux; manual setup for Windows |
| 1.3 Pre-Integration | Read SDK docs and product docs | SDK reminders, integration specs, code examples |
| 1.4 Code Integration | Generate integration code | Self-check 7 items before outputting code |
...
Code examples cover 5 languages and 9 product categories:

| Language | Directory | Products Covered |
|----------|-----------|-----------------|
| Java | `references/code-examples/java/` | All 9 categories |
| Python | `references/code-examples/python/` | Common APIs, Face-to-Face, Order Code, Mobile Web, PC Web, APP |
| Node.js | `references/code-examples/nodejs/` | Common APIs, Face-to-Face, Order Code, Mobile Web, PC Web, APP, Pre-Auth, Merchant Debit |
| PHP | `references/code-examples/php/` | All 9 categories |
| C# | `references/code-examples/csharp/` | All 9 categories |

Product categories: `1_通用接口` (Common APIs), `2_当面付`, `3_订单码支付`, `4_手机网站支付`, `5_电脑网站支付`, `6_JSAPI支付`, `7_APP支付`, `8_预授权支付`, `9_商家扣款`
...
- **全场景支付产品** — 覆盖当面付、订单码支付、APP 支付、JSAPI 支付、手机网站支付、电脑网站支付、预授权支付、商家扣款
...
| 1.2 沙箱环境 | 创建沙箱环境 | Unix/macOS/Linux 使用 CLI 工具；Windows 需手动获取 |
| 1.3 集成前置 | 阅读 SDK 说明与产品文档 | SDK 提醒、接入规范、代码示例 |
| 1.4 集成代码 | 生成集成代码 | 输出代码前须通过 7 项自检 |
...
代码示例覆盖 5 种语言、9 个产品类别：

| 语言 | 目录 | 覆盖产品 |
|------|------|----------|
| Java | `references/code-examples/java/` | 全部 9 个类别 |
| Python | `references/code-examples/python/` | 通用接口、当面付、订单码、手机网站、电脑网站、APP |
| Node.js | `references/code-examples/nodejs/` | 通用接口、当面付、订单码、手机网站、电脑网站、APP、预授权、商家扣款 |
| PHP | `references/code-examples/php/` | 全部 9 个类别 |
| C# | `references/code-examples/csharp/` | 全部 9 个类别 |

产品类别：`1_通用接口`、`2_当面付`、`3_订单码支付`、`4_手机网站支付`、`5_电脑网站支付`、`6_JSAPI支付`、`7_APP支付`、`8_预授权支付`、`9_商家扣款`
