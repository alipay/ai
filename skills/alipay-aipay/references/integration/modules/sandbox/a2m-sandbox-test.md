...
本文档定义按量付费 402 协议的沙箱端到端支付测试流程。测试脚本位于 `../scripts/local_402_sandbox_pay.py`。
...
POST 服务将首行 curl 替换为 `curl -s -D - -o /dev/null -X POST -H "Content-Type: ${CONTENT_TYPE:-application/json}" -d "$BODY" "$SERVICE_URL"`。`seller_signature` 由代码示例中的 `generateSellerSignature` / `generate_seller_signature` 生成，禁止使用空值或占位符。预检输出只打印校验结论和字段名，不打印字段值。
...
**必须将脚本输出的付款链接发给用户**，并提示：

> 请在浏览器打开以下链接，使用沙箱买家账号完成付款：
> - 买家账号：`{email}`（如 `xxxxx@sandbox.com`）
> - 登录密码：`{logonPassword}`（如 `111111`）
> - 支付密码：`{payPassword}`（如 `111111`）
>
> {付款链接}

同时提醒用户：

> 沙箱支付宝当前仅支持安卓系统。如果尚未安装沙箱版支付宝，请复制以下链接到网页浏览器打开，并使用支付宝客户端扫码下载沙箱版支付宝安装包：
>
> https://mdn.alipayobjects.com/sandboxsys/afts/img/_itHRrdOD9oAAAAAAAAAAAAADgSLAQBr/original

等待用户确认付款成功后，记录过程产物目录 `$ARTIFACT_DIR`，进入第四步。

### 第四步：验证 Payment-Proof 重试

用户确认付款成功后，运行 `complete` 命令：
...
**沙箱 Payment-Proof 构建规则**：
- `trade_no` 必须使用前面沙箱收银接口下单返回的交易号，即脚本从 `payResponse.protocol.tradeNo`  提取并写入 `state.json` 的值；不要使用商户侧 `out_trade_no`，也不要重新生成。
- `protocol.payment_proof` 在沙箱环境不做强校验，脚本默认用 `trade_no`、买家 2088 和当前毫秒时间戳生成一个非空 SHA-256 值。
- `method.client_session` 是 `{"externalId":"<BUYER_2088>","signature":"-","timestamp":"<毫秒时间戳>"}` 紧凑 JSON 的 Base64 编码。
- 最终 `Payment-Proof` 请求头值是下面整体 JSON 紧凑序列化后的 Base64 编码。
...
```bash
python3 ../scripts/local_402_sandbox_pay.py run   --url "$SERVICE_URL"   --method GET   --buyer-id "$BUYER_2088"   --payment-needed "$PAYMENT_NEEDED_VALUE"
```
...
6. **沙箱支付宝安装包**：沙箱支付宝当前仅支持安卓系统；如无法完成付款，引导用户复制 `https://mdn.alipayobjects.com/sandboxsys/afts/img/_itHRrdOD9oAAAAAAAAAAAAADgSLAQBr/original` 到网页浏览器打开，并使用支付宝客户端扫码下载沙箱版支付宝安装包
