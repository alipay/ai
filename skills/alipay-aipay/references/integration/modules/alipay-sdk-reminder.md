...
| 手动添加 PEM 头尾、其他前后缀、包装行或说明文字 | 支付宝 SDK 会按配置自行处理密钥包装，手动添加会导致格式错误 | 直接使用沙箱或生产配置中的原始私钥值，不做任何格式处理 |
...
第 1 步（误判起点）：看到 ERR_OSSL_UNSUPPORTED → 推断"私钥格式有问题"
    ↓
第 2 步（错误修复）：认为"PKCS#1 私钥应该手动添加 PEM 头尾或其他前后缀"
    → 手动添加 PEM 头尾或其他前后缀 → 错误仍在
    ↓
